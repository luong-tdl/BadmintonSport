    ...
    public class ShoppingCartController : Controller
    {
        ...
        public async  Task<IActionResult> Index()
        {
            ...
            if(lstShoppingCart.Count>0)
            {
                ...
                {
                    Products prod = _db.Products.Include(p => p.SpecialTags).Include(p => p.ProductTypes).Include(p=>p.Provider).Where(p => p.Id == cartItem).FirstOrDefault();
        ...
        public IActionResult AppointmentConfirmation(int id)
        {
            ...
            {
                ShoppingCartVM.Products.Add(_db.Products.Include(p => p.ProductTypes).Include(p => p.SpecialTags).Include(p=>p.Provider).Where(p => p.Id == prodAptObj.ProductId).FirstOrDefault());
            }
